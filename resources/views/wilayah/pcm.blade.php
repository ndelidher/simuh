@extends('layouts.app')
@section('title', 'Data PCM')

@section('content')
<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;">
    <div>
        <h1 style="font-size:20px;font-weight:500;margin-bottom:4px;">Data PCM</h1>
        <p style="font-size:13px;color:#718096;">Pimpinan Cabang Muhammadiyah · {{ $data->total() }} cabang</p>
    </div>
...
{{-- Filter --}}
<form method="GET" action="{{ route('wilayah.pcm') }}">
<div style="background:#fff;border:0.5px solid #dde8d5;border-radius:12px;padding:12px 16px;display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:12px;">
    <div style="display:flex;flex-direction:column;gap:3px;">
        ...
        <select name="pdm_id" style="height:32px;border:0.5px solid #ccc;border-radius:8px;padding:0 8px;font-size:13px;width:250px;">
            <option value="">Semua PDM</option>
            @foreach($pdmList as $pdm)
            <option value="{{ $pdm->id }}" {{ request('pdm_id') == $pdm->id ? 'selected' : '' }}>
                {{ $pdm->nama }} ({{ $pdm->pwm->nama ?? '' }})
            </option>
            @endforeach
        </select>
    </div>
    ...
    <a href="{{ route('wilayah.pcm') }}" style="border:0.5px solid #ccc;background:transparent;border-radius:8px;padding:0 12px;height:32px;font-size:13px;color:#718096;text-decoration:none;display:flex;align-items:center;">Reset</a>
</div>
</form>
    ...
    <table style="width:100%;border-collapse:collapse;">
        ...
        <tbody>
            ...
            <tr style="border-bottom:0.5px solid #f0f4ec;">
                ...
                <td style="padding:9px 12px;font-size:12px;color:#718096;">{{ $row->pdm->nama ?? '—' }}</td>
                <td style="padding:9px 12px;font-size:12px;color:#718096;">{{ $row->pdm->pwm->nama ?? '—' }}</td>
                <td style="padding:9px 12px;font-size:12px;color:#718096;">{{ $row->kode }}</td>
                <td style="padding:9px 12px;font-size:12px;text-align:center;">{{ $row->prm_list_count }}</td>
            ...
            <tr>
                <td colspan="7" style="padding:40px;text-align:center;color:#718096;font-size:13px;">
                    Belum ada data PCM. <a href="{{ route('wilayah.import') }}" style="color:#3B6D11;">Import dari Excel</a>
                </td>
            </tr>
            @endforelse
        </tbody>
    </table>
    <div style="padding:10px 16px;border-top:0.5px solid #dde8d5;font-size:12px;color:#718096;display:flex;justify-content:space-between;align-items:center;">
        <span>Menampilkan {{ $data->firstItem() ?? 0 }}–{{ $data->lastItem() ?? 0 }} dari {{ $data->total() }} PCM</span>
