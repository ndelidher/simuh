@extends('layouts.app')
@section('title', ($mode=='create'?'Tambah':'Edit') . ' PCM')
@section('content')

<div style="margin-bottom:16px;">
    <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:#718096;margin-bottom:10px;">
        <a href="{{ route('wilayah.pcm') }}" style="color:#3B6D11;text-decoration:none;">Data PCM</a>
        <span>›</span>
        <span>{{ $mode=='create' ? 'Tambah' : 'Edit' }} PCM</span>
    </div>
    <h1 style="font-size:20px;font-weight:500;">{{ $mode=='create' ? 'Tambah' : 'Edit' }} PCM</h1>
</div>

<form method="POST" action="{{ $mode=='create' ? route('wilayah.pcm.store') : route('wilayah.pcm.update', $data) }}">
@csrf
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
        <div>
            ...
            <select id="sel_pwm" onchange="loadPDM(this.value)"
                ...
                <option value="{{ $p->id }}" {{ old('pwm_id', $data?->pdm?->pwm_id ?? '') == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="pdm_id" id="sel_pdm"
                ...
                <option value="{{ $p->id }}" {{ old('pdm_id', $data->pdm_id ?? '') == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                @endforeach
            </select>
            @error('pdm_id')<span style="font-size:11px;color:red;">{{ $message }}</span>@enderror
        </div>
        <div>
...
<div style="display:flex;gap:10px;">
    ...
    <a href="{{ route('wilayah.pcm') }}" style="border:0.5px solid #ccc;background:transparent;border-radius:8px;padding:10px 20px;font-size:14px;color:#718096;text-decoration:none;">Batal</a>
</div>
</form>
@endsection
@push('scripts')
<script>
const existingPdmId = {{ $data?->pdm_id ?? 'null' }};
