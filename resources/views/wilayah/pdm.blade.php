...
<div style="background:#fff;border:0.5px solid #dde8d5;border-radius:12px;padding:12px 16px;display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:12px;">
    <div style="display:flex;flex-direction:column;gap:3px;">
        ...
        <select name="pwm_id" style="height:32px;border:0.5px solid #ccc;border-radius:8px;padding:0 8px;font-size:13px;width:200px;">
            ...
            <option value="{{ $pwm->id }}" {{ request('pwm_id') == $pwm->id ? 'selected' : '' }}>{{ $pwm->nama }}</option>
            @endforeach
        </select>
    </div>
...
<div style="background:#fff;border:0.5px solid #dde8d5;border-radius:12px;overflow:hidden;">
    <table style="width:100%;border-collapse:collapse;">
        <thead>
            <tr>
                ...
                <th style="background:#EAF3DE;color:#27500A;font-size:12px;font-weight:500;padding:10px 12px;border-bottom:0.5px solid #dde8d5;text-align:left;">Nama PDM</th>
                ...
                <th style="background:#EAF3DE;color:#27500A;font-size:12px;font-weight:500;padding:10px 12px;border-bottom:0.5px solid #dde8d5;text-align:center;width:80px;">Jml PCM</th>
        ...
        <tbody>
            ...
            <tr>
                <td colspan="6" style="padding:40px;text-align:center;color:#718096;font-size:13px;">
                    Belum ada data PDM. <a href="{{ route('wilayah.import') }}" style="color:#3B6D11;">Import dari Excel</a>
                </td>
            </tr>
            @endforelse
        </tbody>
    ...
    <div style="padding:10px 16px;border-top:0.5px solid #dde8d5;font-size:12px;color:#718096;display:flex;justify-content:space-between;align-items:center;">
        <span>Menampilkan {{ $data->firstItem() ?? 0 }}–{{ $data->lastItem() ?? 0 }} dari {{ $data->total() }} PDM</span>
