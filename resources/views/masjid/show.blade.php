@extends('layouts.app')
@section('title', 'Detail Masjid — ' . $masjid->nama)
    ...
    <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div>
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                <h1 style="font-size:18px;font-weight:500;margin:0;">{{ $masjid->nama }}</h1>
                @if($masjid->kategori_unggulan)
                    @php $kat = [
                    ...
                    ][$masjid->kategori_unggulan] ?? null; @endphp
                    @if($kat)
                    <span style="background:{{ $kat['bg'] }};color:{{ $kat['c'] }};font-size:11px;padding:2px 8px;border-radius:20px;">{{ $kat['l'] }}</span>
                    @endif
                @endif
                <span style="background:{{ $masjid->aktif ? '#EAF3DE' : '#F7C1C1' }};color:{{ $masjid->aktif ? '#27500A' : '#791F1F' }};font-size:11px;padding:2px 8px;border-radius:20px;">
                    {{ $masjid->aktif ? 'Aktif' : 'Non-aktif' }}
                </span>
            </div>
            <div style="font-size:12px;color:#718096;">
                {{ $masjid->prm->nama ?? '-' }} ·
                {{ $masjid->prm->pcm->nama ?? '-' }} ·
                {{ $masjid->prm->pcm->pdm->nama ?? '-' }} ·
                {{ $masjid->prm->pcm->pdm->pwm->nama ?? '-' }}
            </div>
        </div>
    </div>
    <div style="display:flex;gap:8px;">
        @if(!auth()->user()->isAdminPP())
        <a href="{{ route('masjid.edit', $masjid) }}"
        ...
        @endif
        <a href="{{ route('indikator.input', $masjid) }}"
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px;">
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:22px;font-weight:500;color:#1C4A2A;">{{ $dataIndikator['JAMAAH_1']->nilai ?? '—' }}</div>
        <div style="font-size:11px;color:#718096;margin-top:4px;">Rata-rata Jamaah Subuh</div>
    </div>
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:22px;font-weight:500;color:#1C4A2A;">{{ $dataIndikator['JAMAAH_2']->nilai ?? '—' }}</div>
    ...
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:22px;font-weight:500;color:#1C4A2A;">
            @if(isset($dataIndikator['JARIYAH_1']))
                Rp {{ number_format($dataIndikator['JARIYAH_1']->nilai, 2, ',', '.') }} jt
            @else—@endif
        </div>
        <div style="font-size:11px;color:#718096;margin-top:4px;">Infaq Bulan Ini (jt)</div>
    </div>
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:10px;padding:14px;text-align:center;">
        <div style="font-size:22px;font-weight:500;color:#1C4A2A;">{{ $dataIndikator['JAMIYAH_3']->nilai ?? '—' }}</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
    ...
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:12px;padding:16px;">
        ...
        @foreach($rows as $r)
        @if($r[1] === null)
        <div style="padding:8px 0 4px;font-size:11px;color:#718096;font-weight:500;border-bottom:0.5px solid #f0f4ec;">{{ $r[0] }}</div>
        @else
        <div style="display:flex;padding:6px 0;border-bottom:0.5px solid #f0f4ec;font-size:12px;">
            <span style="color:#718096;width:120px;flex-shrink:0;">{{ $r[0] }}</span>
            <span style="color:#1a202c;">{{ $r[1] }}</span>
        </div>
        @endif
        @endforeach
    </div>

    {{-- Pemenuhan Indikator Bulan Ini --}}
    <div style="background:#fff;border:0.5px solid #dde8d5;border-radius:12px;padding:16px;">
        <div style="font-size:13px;font-weight:500;color:#1C4A2A;margin-bottom:12px;padding-bottom:8px;border-bottom:0.5px solid #eee;">
            📊 Pemenuhan Indikator — {{ ["","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"][$bulan] }} {{ $tahun }}
        </div>
        @php
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:0.5px solid #f0f4ec;">
            <span style="background:{{ $badge[$ind->kelompok][0] }};color:{{ $badge[$ind->kelompok][1] }};font-size:10px;padding:1px 6px;border-radius:10px;flex-shrink:0;width:60px;text-align:center;">{{ $ind->kode }}</span>
            <div style="flex:1;min-width:0;">
                <div style="font-size:11px;color:#4a5568;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $ind->nama }}</div>
                <div style="height:6px;background:#f0f4ec;border-radius:3px;margin-top:3px;">
                    <div style="height:100%;width:{{ $val ? '80%' : '0%' }};background:{{ $warna[$ind->kelompok] }};border-radius:3px;"></div>
                </div>
            </div>
            <span style="font-size:12px;font-weight:500;color:#1a202c;min-width:50px;text-align:right;">
                {{ $val !== null ? number_format($val, 1) . ' ' . $ind->satuan : '—' }}
            </span>
        </div>
        @endforeach
    </div>
